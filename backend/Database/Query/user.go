...
func CreateUser(db *sql.DB, u *models.User) error {
	...
	if err != nil {
		fmt.Println("error executing query test", err)
...
func GetUserByUUID(db *sql.DB, UUID string, user *models.User) error {
	stmt, err := db.Prepare("SELECT uuid, email, private, first_name, last_name, birth_date, COALESCE(users.nickname, '') AS user_name, COALESCE(about_me,'') AS about_me, COALESCE(img_path,'') AS img_path FROM users WHERE uuid = ?")
	if err != nil {
		fmt.Println(err)
		return errors.New("error preparing query")
	}
